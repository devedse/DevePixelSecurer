...
$7zFilePath = Join-Path $outputDir "DevePixelSecurer.7z"
$zipFilePath = Join-Path $outputDir "DevePixelSecurer.zip"

DeleteFileIfExists $7zFilePath
DeleteFileIfExists $zipFilePath
DeleteFolderIfExists $outputDir

$buildPath = Join-Path $solutionRoot "DevePixelSecurer\bin\Release\netstandard2.0"
